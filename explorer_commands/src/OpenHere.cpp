...
static std::filesystem::path GetSublimeMergePath() {
    LOG(L"GetSublimeMergePath");
    ...
    std::filesystem::path sublime_path = std::filesystem::path(module_path.data()).parent_path().parent_path().append("sublime_merge.exe");
...
{   
    LOG(L"Invoke");
    try {
        auto exe_path = GetSublimeMergePath();
        ...
        if (item) {
            ...
            if (SUCCEEDED(item->GetDisplayName(SIGDN_FILESYSPATH, &name))) {
                ...
                /* std::wstring cmd_line = L"\"" + std::wstring(name.get()) + L"\"";
                ...
                std::wstring cmd_line = L"\"" + exe_path.wstring() + L"\" \"" + std::wstring(name.get()) + L"\"";
...
{
    LOG(L"GetTitle");
    const std::wstring resource = L"Open in Sublime Merge";
    ...
    try {
        ...
        if (item) {
            ...
            if (is_file_system_folder && !is_compressed) {
                wil::unique_cotaskmem_string name;
                if (SUCCEEDED(item->GetDisplayName(SIGDN_FILESYSPATH, &name))) {
                    auto git_path = std::filesystem::path(name.get()).append(".git");
...
IFACEMETHODIMP OpenHere::GetIcon(IShellItemArray*, LPWSTR* ppszIcon)
{
    LOG(L"GetIcon");
    try {
        auto location = GetSublimeMergePath();
